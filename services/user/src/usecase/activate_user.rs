...
mod tests {
    ...
    async fn should_activate_user_when_caller_has_higher_role() {
        ...
        let target = make_user(target_id, UserRole::User, false); // inactive
    ...
    async fn should_return_insufficient_role_for_activate_when_caller_has_equal_or_lower_role() {
        ...
        let payload = ActivateUserPayload {
            target_id,
            caller_id,
            caller_role: UserRole::Admin, // equal role
        };
    ...
    async fn should_return_user_already_active_when_user_is_already_active() {
        let caller_id = Uuid::new_v4();
        let target_id = Uuid::new_v4();
        let target = make_user(target_id, UserRole::User, true); // already active
        let target_clone = target.clone();
